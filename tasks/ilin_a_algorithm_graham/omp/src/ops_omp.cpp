...
IlinAGrahamOMP::IlinAGrahamOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool IlinAGrahamOMP::ValidationImpl() {
  return !GetInput().points.empty();
}

bool IlinAGrahamOMP::PreProcessingImpl() {
  points_ = GetInput().points;
  hull_.clear();
  return true;
}

bool IlinAGrahamOMP::RunImpl() {
  if (points_.size() < 3) {
    hull_ = points_;
    return true;
  }
...
bool IlinAGrahamOMP::PostProcessingImpl() {
  GetOutput() = hull_;
  return true;
}
